...
if [[ "$1" == "remove" ]]; then
  docker exec -it `docker ps | grep devops-utils | awk '{print $1}'` \
      sh /vagrant/tz-local/docker/init2.sh && bash /vagrant/scripts/eks_remove_all.sh
  if [[ $? != 0 ]]; then
    echo "failed to remove resources!"
    exit 1
  fi
  docker exec -it `docker ps | grep devops-utils | awk '{print $1}'` \
      sh /vagrant/scripts/eks_remove_all.sh cleanTfFiles
  exit 0
fi
...
docker exec -it `docker ps | grep devops-utils | awk '{print $1}'` \
    sh /vagrant/tz-local/docker/init2.sh
