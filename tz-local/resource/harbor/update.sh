...
cd /vagrant/projects/devops-web
...
GIT_BRANCH=k8s
GIT_URL="https://github.com/tzkr/devops-web.git"
TAG_ID="latest"
DOCKER_NAME="devops-web"
DOCKER_NAME_NGINX="devops-nginx"
...
REPO_HOST="harbor.default.eks-main.tzcorp.com"
...
{
  "insecure-registries":["harbor.default.${eks_project}.${eks_domain}"]
}
sudo systemctl restart docker

echo https://harbor.default.${eks_project}.${eks_domain}
docker login harbor.default.${eks_project}.${eks_domain} -u="admin" -p="${admin_password}"
#docker login ${REPO_HOST} -u="admin" -p="${admin_password}"

#docker container stop $(docker container ls -a -q) && docker system prune -a -f --volumes
docker build -f docker/local/devopsBase.Dockerfile -t ${REPO_HOST}/devops-web-phpbase/repository:latest .
docker push ${REPO_HOST}/devops-web-phpbase/repository:latest

docker build -f docker/local/devopsWeb.Dockerfile -t ${REPOSITORY_TAG} .
docker push ${REPOSITORY_TAG}

docker build -f docker/local/devopsNginx.Dockerfile -t ${REPOSITORY_TAG_NGINX} .
